...
int fast_flash_init(fl_QSPIPorts &qspi) {
    ...
    char results[6*10];  // clk_divider is at most 4
...
void fast_flash_xip(fl_QSPIPorts &qspi, unsigned addr, unsigned read_count, unsigned read_data[], chanend ?c_data_out) {
    fast_read_loop(qspi, addr, 0, best_read_adj, read_count, read_data, c_data_out);
}
