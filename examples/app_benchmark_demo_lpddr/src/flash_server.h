...
typedef enum flash_command {
  ...
  FLASH_READ_XIP =
      3, ///< Read code to execute-in-place throught L2 cache
  FLASH_READ_XIP_DIFF_TILE =
      4, ///< Read code to execute-in-place throught L2 cache
  FLASH_SERVER_QUIT = 5,
  FLASH_SERVER_INIT = 6, // Initialize flash server with fast flash pattern speed match setup
...
#else
DECLARE_JOB(flash_server, (chanend_t*, flash_t*, int, fl_QSPIPorts *, fl_QuadDeviceSpec*, int) );
